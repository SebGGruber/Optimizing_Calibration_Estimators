# author: teodora popordanoska
# source: https://github.com/tpopordanoska/ece-kde/blob/main/ece_kde.py
import torch
import numpy as np
...
#################################
# from here on, author: Anon First-Author
#################################
def dirichlet_kernel_i(z, zi, bandwidth=0.1):
    alphas = zi / bandwidth + 1
...
def get_kernel_i(fi, f, bandwidth, device):
    # if num_classes == 1
    if f.shape[1] == 1:
        log_kern = beta_kernel(zi=fi, z=f, bandwidth=bandwidth).squeeze()
    else:
        log_kern = dirichlet_kernel_i(zi=fi, z=f, bandwidth=bandwidth).squeeze()
    # Trick: -inf on the diagonal
    #return log_kern + torch.diag(torch.finfo(torch.float).min * torch.ones(len(f))).to(device)
    return log_kern

def get_ratio_canonical_i(fi, yi, f, bandwidth, device):
    ...
    if f.shape[1] > 60:
        # Slower but more numerically stable implementation for larger number of classes
        return get_ratio_canonical_log_i(fi, yi, f, bandwidth, device)

    log_kern = get_kernel_i(f=f, fi=fi, bandwidth=bandwidth, device=device)
    kern = torch.exp(log_kern)

    y_onehot = nn.functional.one_hot(yi, num_classes=fi.shape[1]).to(torch.float32)
    kern_y = torch.matmul(kern, y_onehot)
    den = torch.sum(kern, dim=1)
    # to avoid division by 0
    den = torch.clamp(den, min=1e-10)
...
# Note for training: Make sure there are at least two examples for every class present in the batch, otherwise
# LogsumexpBackward returns nans.
def get_ratio_canonical_log_i(fi, yi, f, bandwidth, device='cpu'):
    log_kern = get_kernel_i(f=f, fi=fi, bandwidth=bandwidth, device=device)
    y_onehot = nn.functional.one_hot(yi, num_classes=fi.shape[1]).to(torch.float32)
    log_y = torch.log(y_onehot)
    log_den = torch.logsumexp(log_kern, dim=1)
    final_diff = torch.zeros(f.shape)
    for k in range(fi.shape[1]):
        log_kern_y = log_kern + (torch.ones([f.shape[0], 1]) * log_y[:, k].unsqueeze(0))
        log_inner_ratio = torch.logsumexp(log_kern_y, dim=1) - log_den
        inner_ratio = torch.exp(log_inner_ratio)
        inner_diff = inner_ratio - f[:, k]
        final_diff[:, k] = inner_diff

    return final_diff

# def get_ratio_marginal_vect_i(fi, yi, f, bandwidth, p, device):
#     y_onehot = nn.functional.one_hot(yi, num_classes=fi.shape[1]).to(torch.float32)
#     log_kern_vect = beta_kernel(zi=fi, z=f, bandwidth).squeeze()
#     #log_kern_diag = torch.diag(torch.finfo(torch.float).min * torch.ones(len(f))).to(device)
#     # Multiclass case
#     #log_kern_diag_repeated = f.shape[1] * [log_kern_diag]
#     #log_kern_diag_repeated = torch.stack(log_kern_diag_repeated, dim=2)
...
#     return get_kde_for_ece_vect_i(f, y_onehot, log_kern_vect)

# def get_ratio_toplabel_i(fi, yi, f, bandwidth, p, device):
#     f_max, indices = torch.max(f, 1)
#     f_max = f_max.unsqueeze(-1)
#     y_max = (y == indices).to(torch.int)
...
def get_kde_for_ece_i(fi, yi, f, log_kern):
    f = f.squeeze()
    fi = fi.squeeze()
    N = len(f)
    # Select the entries where y = 1
    idx = torch.where(yi == 1)[0]
...
def get_ratio_binary_i(fi, yi, f, bandwidth, device):
    ...
    log_kern = get_kernel_i(fi, f, bandwidth, device)

    return get_kde_for_ece_i(fi, yi, f, log_kern)

def ECE_kde_fit(Ps, Ys, bw, notion='binary'):

    def cal_model(new_Ps):
        if len(Ps.shape)>1 and Ps.shape[1]>1:
            diff_preds = get_ratio_canonical_i(Ps, Ys, new_Ps, bandwidth=bw, device='cpu').numpy()
            return diff_preds @ diff_preds.T
        else:
            diff_preds = get_ratio_binary_i(Ps, Ys, new_Ps, bandwidth=bw, device='cpu').numpy()
